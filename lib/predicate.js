 ...
 * @return {function}
 */
const $eq = opts => value => _([]).concat(opts).some(v => v === value);
...
const $gt = opts => value => _([]).concat(opts).some(v => value > v);

const $gte = opts => value => _([]).concat(opts).some(v => value >= v);

const $lt = opts => value => _([]).concat(opts).some(v => value < v);

const $lte = opts => value => _([]).concat(opts).some(v => value <= v);
 ...
 * @returns {function}
 */
const $between = excludeEnds => (opts, lenient) => $and([(excludeEnds ? $gt : $gte)(_.min(_.map(opts, v => +v)), lenient), (excludeEnds ? $lt : $lte)(_.max(_.map(opts, v => +v)), lenient)], lenient);
...
const $like = exprs => {
    ...
    return value => _.some(patterns, regex => regex.test(value));
};
...
const $contains = opts => value => {
    ...
    return _([]).concat(opts).uniq().every(v => _.includes(testVal, v));
};
...
const $and = (opts, lenient) => {
    ...
    return value => _.every(predicates, p => p(value));
};
...
const $or = (opts, lenient) => {
    const predicates = opts.map(opt => predicate(opt, lenient));
    return value => _.some(predicates, p => p(value));
};
...
function predicate (p = {}, lenient = false) {
    ...
    return v => _.every(predicates, p => p(v));
}
