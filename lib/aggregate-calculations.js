...
internals.parse = function (fields, expr, config) {
    ...
    let blob = config.pattern.exec(expr)[1];
    //split on the first comma not in a parentheses
    let args = internals.splitFunction(blob);
    let arg1 = AggregateExpression.AggregateExpression.parse(fields, args[0].trim());
    let arg2 = AggregateExpression.AggregateExpression.parse(fields, args[1].trim());
...
parsers.constant = {
    parse: function (fields, expr) {
        joi.assert(expr, joi.string().regex(/[0-9]/));
...
exports.parseExpression = function (fields, expr) {
    return _.values(parsers).reduce(function (match, element) {
        if (!match) {
            let attempt = _.attempt(element.parse, fields, expr);
