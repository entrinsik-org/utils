...
internals.valids = function (fields) {
    return _(fields)
        ...
        .reduce(function (acc, f) {
            ...
            return acc;
        }, [COUNT].concat(_.map(fields, f => `${VALUE_COUNT}:${f.name}`)));
};
 ...
 * @param {string[] =} fns an optional array of functions to be considered (defaults to ['count', 'sum', 'avg', 'max', 'min', 'value_count'])
 * @return {Array.<T>}
 */
AggregateExpression.of = function (fields, fns) {
    fns = fns || [COUNT, SUM, AVG, MAX, MIN, VALUE_COUNT];
    ...
    return countExprs.concat(numberExprs).concat(dateExprs).concat(valueCountExprs);
};
