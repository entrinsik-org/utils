...
const AggregationCalculation = require('./aggregate-calculations');
...
AggregateExpression.prototype.toString = function () {
    return this.fn === COUNT ? COUNT : this.field ? `${this.fn}:${this.field.name}` : this.fn;
};
...
AggregateExpression.prototype.aggName = function () {
    return this.fn === COUNT ? COUNT : this.field ? _.snakeCase(`${this.fn}-${this.field.name}`) : _.snakeCase(this.fn);
};
...
AggregateExpression.prototype.apply = function (agg, mapper) {
    ...
    if (agg.body.missing) {
        let missingFn = this.fn;
        if (missingFn !== COUNT) {
            agg.aggregation(aggId, { [this.fn]: { field: this.field && this.field.name } })
        ...
        if (this.fn !== COUNT) {
            agg.aggregation(aggId, { [this.fn]: this.field ? { field: this.field.name } : {} })
...
AggregateExpression.parse = function (fields, expr) {
    // attempt at parsing expr as one of the aggregate calculations.
    let calc = AggregationCalculation.parseExpression(fields, expr);
    if(calc !== null) {
        return calc;
    }
...
/**
 * Determines whether or not the expression is a calculation or built in aggregate expression
 * @param fields
 * @param expr
 * @returns {boolean}
 */
AggregateExpression.isCalculation = function (fields, expr) {
    return AggregationCalculation.parseExpression(fields, expr) !== null;
};
