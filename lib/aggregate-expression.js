 ...
 *
 * Valid fields are any objects with properties { name: string, label: string, isNumeric(), isDate(), isString(), isBoolean() }
...
const COUNT = 'count', SUM = 'sum', AVG = 'avg', MAX = 'max', MIN = 'min', VALUE_COUNT = 'value_count', CARDINALITY = 'cardinality', RATIO = 'ratio';

const ratioRegex =  /ratio\(([^]+)\)/;
 ...
 * @param {{ name: string, label: string, isDate: function, isNumeric: function, isBoolean: function, isString: function}=} field the field being aggregated
 * @param array of nested aggregate functions
 * @constructor
 */
function AggregateExpression (fn, field, nested) {
    this.fn = fn;
    this.field = field;
    this.nested = nested;
}
...
AggregateExpression.prototype.label = function () {
    switch (this.fn) {
        ...
        case RATIO:
            return `${this.nested[0].label()} / ${this.nested[1].label()}`;
        default:
            return 'Unknown';
    }
};
...
AggregateExpression.prototype.toString = function () {
    if(this.fn === RATIO)
        return `ratio(${this.nested[0].toString()}, ${this.nested[1].toString()})`;
...
AggregateExpression.prototype.sort = function (agg, direction) {
    if (this.fn !== COUNT && this.fn !== RATIO) agg.configure({ order: { [this.aggName()]: direction || 'desc' } });
    return this;
};
...
AggregateExpression.prototype.aggName = function () {
    if(this.fn === RATIO) {
        return `ratio-${this.nested[0].aggName()}-${this.nested[1].aggName()}`;
    }
    return this.fn === COUNT ? COUNT : `${this.fn}-${this.field.name}`;
};
    ...
    if (agg.body.missing) {
        ...
        switch (this.fn) {
            case COUNT:
                ...
                break;
            case RATIO:
                //apply each of the component aggs
                this.nested[0].apply(agg, mapper);
                this.nested[1].apply(agg, mapper);
                let ratBody = _.set({}, 'bucket_script.buckets_path.numer', this.nested[0].fn === COUNT ? '_count': this.nested[0].aggName());
                _.set(ratBody, 'bucket_script.buckets_path.denom', this.nested[1].fn === COUNT ? '_count': this.nested[1].aggName());
                _.set(ratBody, 'bucket_script.script', 'params.numer / params.denom');
                agg.aggregation(aggId, ratBody)
...
AggregateExpression.parse = function (fields, expr) {
    joi.assert(expr, [joi.string().valid(internals.valids(fields)), joi.string().regex(ratioRegex)]);
    if(expr.startsWith(RATIO)) {
        //need internal validation of the ratio function (has 2 args, they are valid, etc)
        let blob = ratioRegex.exec(expr)[1];
        //split on the first comma not in a parentheses
        let args = internals.splitFunction(blob);
        let numer = this.parse(fields, args[0].trim());
        let denom = this.parse(fields, args[1].trim());
        return new AggregateExpression('ratio',null,[numer,denom]);
    }
...
AggregateExpression.of = function (fields, fns) {
    ...
    let numberExprs = _.filter(fields, f => f.isNumeric() )
        .reduce(function (acc, field) {
            return acc.concat(numberFns.map(fn => new AggregateExpression(fn, field)));
        }, []);

    let dateExprs = _.filter(fields, f => f.isDate())
        .reduce(function (acc, field) {
