 ...
 * @param {boolean} disableSort disables applying default sort (some bucket aggs dont support sorting)
 * @return {*}
 */
AggregateExpression.prototype.apply = function(agg, mapper, disableSort) {
    ...
    if (this.fn !== COUNT) {
        ...
        if (!disableSort) agg.configure({ order: { [aggId]: 'desc' }});
    } else {
